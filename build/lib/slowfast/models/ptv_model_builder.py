...
from .ae_helper import create_resnet_encoder, create_resnet_decoder, ResNetAutoencoder
...
class PTVResNetAutoencoder(nn.Module):
    ...
    def _construct_network(self,cfg):
        ...
        resnet = create_resnet_encoder(
            # Input clip configs.
            input_channel=cfg.DATA.INPUT_CHANNEL_NUM[0],

            # Model configs.
            model_depth=18,#cfg.RESNET.DEPTH,
            model_num_class=cfg.MODEL.NUM_CLASSES,
            dropout_rate=cfg.MODEL.DROPOUT_RATE,
            # Normalization configs.
            norm=norm_module,
            ...
            bottleneck=create_bottleneck_block,
            # Head configs.
            head=None,#create_encoder_head,
            head_pool=nn.AvgPool3d,
            head_pool_kernel_size=(
                cfg.DATA.NUM_FRAMES // (pool_size[0][0]*2),
                cfg.DATA.TRAIN_CROP_SIZE // 32 // (pool_size[0][1]*2),
                cfg.DATA.TRAIN_CROP_SIZE // 32 // (pool_size[0][2]*2),
            ),
            head_output_size=(2,2,2),
            head_activation=None,
            conv=nn.Conv3d
        )
        self.model = ResNetAutoencoder(resnet, [1, 1, 1, 1],n_channels=cfg.DATA.INPUT_CHANNEL_NUM[0])

    def forward(self, x):
        x = x[0]
        x = self.model(x)[1] #get just x for training, not z
        return x
