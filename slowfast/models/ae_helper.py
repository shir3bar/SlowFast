...
def create_resnet_encoder(
    *,
    ...
    # Head configs.
    head: Callable = create_encoder_head,
...
class Bridge(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.bridge_down = nn.Sequential(
            ...
            nn.AvgPool3d(kernel_size=(32, 8, 8), stride=(1, 1, 1), padding=(0, 0, 0)), )
        self.bridge_up = nn.Sequential(
...
class UpBlockForUNetWithResNet50(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, up_conv_in_channels=None, up_conv_out_channels=None,
        ...
        if upsampling_method == "conv_transpose":
            self.upsample = nn.ConvTranspose3d(up_conv_in_channels, up_conv_out_channels, kernel_size=(1, 2, 2),
                                               stride=(1, 2, 2))
...
class UNetWithResnet50(nn.Module):
    ...
    def __init__(self, resnet, n_channels=3):
        super().__init__()
        down_blocks = []
        up_blocks = []
        stem = resnet.model.blocks[0]
        ...
        for bottleneck in list(resnet.model.blocks.children()):
        ...
        up_blocks.append(UpBlockForUNetWithResNet50(in_channels=16 + 8, out_channels=16,
                                                    up_conv_in_channels=32, up_conv_out_channels=16))
        up_blocks.append(UpBlockForUNetWithResNet50(in_channels=8 + n_channels, out_channels=8,
                                                    up_conv_in_channels=16, up_conv_out_channels=8))

        self.up_blocks = nn.ModuleList(up_blocks)

        self.out = nn.Conv3d(8, n_channels, kernel_size=1, stride=1)
    ...
    def forward(self, x, with_output_feature_map=False):
        ...
        for i, block in enumerate(self.up_blocks, 1):
            key = f"layer_{UNetWithResnet50.DEPTH - 1 - i}"
